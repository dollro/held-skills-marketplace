# Component Recipes for Penpot MCP

Reusable code patterns for generating UI components via `mcp__penpot__execute_code`.
Each recipe is a self-contained template — adapt values based on the Confirmed Spec.

> **Execution constraints reminder:**
...
```javascript
// Configuration — replace with Confirmed Spec values
const colors = {
  'Primitives': [
...
When the Confirmed Spec uses a dark color scheme, invert the patterns:
...
If the screenshots show a component not covered by these recipes:
