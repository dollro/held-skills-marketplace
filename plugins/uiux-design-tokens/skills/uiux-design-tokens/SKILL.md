...
Design tokens are the atomic values of a design system — colors, spacing, typography, shadows, and more — stored as platform-agnostic data. They bridge the gap between design tools and code, ensuring consistency across platforms and products.
...
| Need a quick starting point to adapt | `references/starter-template.md` |
...
For complex tasks (e.g. building a full token set from scratch for a new project), you may need all three references. Load `starter-template.md` first for scaffolding, then `dtcg-format.md` for validation, and finally `platform-mapping.md` when ready to output.
