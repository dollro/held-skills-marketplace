# Color Utilities for Penpot

Penpot only accepts hex color strings (`#RRGGBB`). CSS color functions like `oklch()`, `hsl()`, `rgb()` are **not supported**. Use these pure-math converters inside `execute_code`.

> **IMPORTANT:** The `execute_code` context has no DOM. `document.createElement`, `getComputedStyle`, and similar browser APIs will fail with "Cannot read properties of undefined". All conversions must be pure JavaScript math.
